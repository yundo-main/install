...
├── 02-key-transfer.md          Mac→노드: .pub 전달 (스크립트 없음 — 부트스트랩이라 자동화 불가)
├── 03-ssh-keys.md   / .sh      노드 로컬: 공개키를 authorized_keys 에 등록
├── 04-ssh-client.md / .sh      macOS: 호스트 키 지문 대조·known_hosts·config·접속 검증
│
...
| [`02-key-transfer.md`](02-key-transfer.md) | `.pub` 을 노드로 전달 (콘솔 붙여넣기 등) — 스크립트 없음 | Mac → 대상 노드 |
| [`03-ssh-keys.md`](03-ssh-keys.md) / [`.sh`](03-ssh-keys.sh) | 공개키 등록 (이 키를 신뢰한다) | 대상 노드 (로컬) |
| [`04-ssh-client.md`](04-ssh-client.md) / [`.sh`](04-ssh-client.sh) | 지문 대조·known_hosts·config·접속 검증 | macOS 클라이언트 |
| [`controls.md`](controls.md) | 통제 원칙·대조 상수·잔여 위험 색인 | (참조) |

`00`(정책)과 `03`(키 등록)을 분리한 이유: 정책은 노드마다 한 번 고정되지만 키는
회전·추가·폐기가 그보다 잦다. 방화벽·소켓 설정을 다시 건드리지 않고 키만 갱신할
수 있어야 한다. `01`(키 생성)·`02`(전달)·`03`(등록)이 각각 분리된 이유는 신뢰
경계가 다르기 때문이다 — 아래 「스크립트 분리 기준」 참조.
...
- `NN-<concept>.{sh,md}` 페어, 같은 번호. 디렉터리 목록만 보고 실행 순서를 알 수
  있어야 한다.
- **예외**: `02-key-transfer.md` 는 `.sh` 가 없다. 부트스트랩 이전이라 자동화할
  채널 자체가 없는 수동 단계이기 때문이다 — 절차 문서만으로 완결한다.
...
| 02 | Mac → 대상 노드 | [02-key-transfer.md](02-key-transfer.md) |
| 03 | 대상 노드 (콘솔/로컬) | [03-ssh-keys.md](03-ssh-keys.md) |
| 04 | macOS 클라이언트 | [04-ssh-client.md](04-ssh-client.md) |

재확인은 각 스크립트의 `--verify-only`.

## 스크립트 분리 기준

**실행 주체의 신뢰 경계가 다르면 파일을 나눈다.** 노드 로컬(`00`, `03`)과
macOS 클라이언트(`01`, `04`)를 한 파일에 두지 않는다. 노드로 전송되는
스크립트(`00`, `03`)는 클라이언트 로직을 포함하지 않고, 외부 파일을 참조하지
않는 자기완결 스크립트여야 한다.

`01`(키 생성)·`02`(전달)·`03`(등록)도 이 기준으로 나뉜다 — 개인키는 `01`을
실행한 Mac 을 벗어나지 않고, `02`는 그 `.pub` 을 신뢰 없는 채널로 옮기는
수작업이며, `03`은 노드에 이미 도착한 공개키만 받는다. 세 단계를 하나로
합치면 "개인키가 어디까지 갔는지"와 "자동화 가능 구간이 어디까지인지"가
한 파일 안에 섞인다.
