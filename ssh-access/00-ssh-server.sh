...
#
# 관계: 03-ssh-keys.sh 는 authorized_keys 배치(키 등록)를 담당한다. 04-ssh-client.sh
...
공개키 등록(authorized_keys)은 이 스크립트가 아니라 03-ssh-keys.sh 가 한다.
...
$(ssh-keygen -lf "$HOST_KEY_PUB" 2>/dev/null | sed 's/^/    /')
  ...
  다음: 03-ssh-keys.sh 로 공개키를 authorized_keys 에 등록한다 (아직 배치 전이면
