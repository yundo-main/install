...
> 실행 도구는 [`02-ssh-keys.sh`](02-ssh-keys.sh). 키 생성·전달은
> [01-key-setup.md](01-key-setup.md)(Mac). sshd·방화벽·인증 정책과 접속 검증은
> [00-ssh-server.md](00-ssh-server.md)(수동 절차, 첫 SSH 접속·지문 대조 포함).

`00-ssh-server.md` (수동 절차)가 "누가·어떻게 인증할 수 있는가"(정책)를 정하고, 이 단계는
...
이 단계의 전제는 [01-key-setup.md](01-key-setup.md) 로 `.pub` 파일이 이미
노드에 와 있는 것이다.
...
- 키를 생성하지 않는다. 개인키의 생성·보관·전달은
  [01-key-setup.md](01-key-setup.md)·운영자 책임이다.
...
- `authorized_keys` 를 이 스크립트가 배타적으로 관리하지 않는다. 손으로
  `ssh-copy-id` 등을 병행하면 같은 파일에 쓸 수 있다 — 위 옵션 충돌 검사로
  걸러지지만, 콘솔 접근이 되면 이 스크립트로 통일한다.
